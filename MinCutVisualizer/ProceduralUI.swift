...
extension RandomGraphElements {
    func generateRandomNumVerticesWithColors(_ numVertices: Int) -> [Vertex<UIColor>] {
        ...
        if colors.count > numVertices {
            colors = Array(colors[0..<numVertices])
        }
//        colors = colors.count > numVertices ? Array(colors[0..<numVertices]) : colors
        return generateVertices(withValues: colors)
    }
    
    private func generateVertices<T>(withValues values: [T]) -> [Vertex<T>] {
    ...
    func generateRandomUndirectedEdges<T>(betweenVertices vertices: [Vertex<T>]) -> [Edge<T>] {
        ...
        var allVertexAppearances = initalVerticesCounts(vertices)
    ...
    func initalVerticesCounts<T>(_ vertices: [Vertex<T>]) -> [(vertex: Vertex<T>, num: Int)] {
        guard vertices.count > 1 else { return [] }
        // select random total number of edges between 1.25 * vertices.count and 2 * vertices.count
        ...
        // list of all vertex appearances, start with 2 of each vertex, to minimize multiple edges and chances of self-loops
        var allVertexAppearances: [(vertex: Vertex<T>, num: Int)] = vertices.map { ($0, 2) }
        // then 2 * (numEdges - vertices.count) are chosen randomly
        for _ in 0..<(numVerticeAppearances-(vertices.count*2)) {
            let randomIdx = Int.random(in: 0..<vertices.count)
            allVertexAppearances[randomIdx].num += 1
        }

        return allVertexAppearances
    }
